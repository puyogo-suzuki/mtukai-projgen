...
impl EntryMacroArgs {
    ...
    fn infer_lp_path(archname :&String) -> syn::Result<String> {
        ...
        .and_then(|md| md.get("mtukai"))
        .and_then(|mt| mt.as_table()) {
            if let Some(lp_path_value) = metadata.get("lp_path").and_then(|lp_path| lp_path.as_str()) {
                if let Some(lp_bin_path) = Path::new(&lp_path_value).join("target").join(archname).join("release").join("main").to_str() {
                    ...
                    Err(Error::new(Span::call_site().into(), "The string value of package.metadata.mtukai.lp_path in Cargo.toml is invalid."))
                }
            } else {
                Err(Error::new(Span::call_site().into(), "The string value of package.metadata.mtukai.lp_path not found in Cargo.toml. Please make sure to include the lp_path field in the mtukai metadata section of your Cargo.toml, pointing to the LP binary generated by the build script."))
            }
        } else {
            Err(Error::new(Span::call_site().into(), "package.metadata.mtukai section not found in Cargo.toml. Please make sure to include the mtukai metadata section in your Cargo.toml."))
        }
    }
}
