...
impl EntryMacroArgs {
    ...
    fn infer_lp_path(archname :&String) -> syn::Result<String> {
        ...
        let metadata = if let Some(metadata) = cargo_toml.get("metadata.mtukai")
            ...
            return Err(Error::new(Span::call_site().into(), "metadata.mtukai section not found in Cargo.toml. Please make sure to include the mtukai metadata section in your Cargo.toml."));
        };
        if let Some(lp_path) = metadata.get("lp_path")
            ...
            if let Some(lp_bin_path) = Path::new(&lp_path_value).join("target").join(archname).join("release").join("main").to_str() {
                ...
                return Err(Error::new(Span::call_site().into(), "The string value of metadata.mtukai.lp_path in Cargo.toml is invalid."));
            }
        } else {
            return Err(Error::new(Span::call_site().into(), "The string value of metadata.mtukai.lp_path not found in Cargo.toml. Please make sure to include the lp_path field in the mtukai metadata section of your Cargo.toml, pointing to the LP binary generated by the build script."));
        }
    }
}
