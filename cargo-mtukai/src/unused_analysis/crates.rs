...
fn parse_disabled_optional_deps_from_toml(
    ...
    for feat in features {
        if all_optional_deps.contains(feat) {
            enabled_deps.insert(feat.clone());
        }
    }
...
"#;
        let disabled_lp = parse_disabled_optional_deps_from_toml(
            toml_content,
            &["esp32c6".to_string(), "is-lp-core".to_string()],
        );
        ...
        let disabled_hp = parse_disabled_optional_deps_from_toml(
            toml_content,
            &["esp32c6".to_string(), "has-lp-core".to_string()],
        );
